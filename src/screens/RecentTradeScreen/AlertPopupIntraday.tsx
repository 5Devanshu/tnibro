      ...
      {({handleSubmit, setFieldValue, values, errors}) => (
        <View>
          ...
          <View style={styles.formAction}>
            ...
            {isSetAlertError && (
              <Text style={{color: COLORS.red, marginTop: 5}}>{isSetAlertError?.response?.replace("stock", "Index")}</Text>
            )}
              <View style={{
...
const PriceAlert = (props: any) => {
  ...
  //       type: 'error',
  //       text1: 'Alert Msg',
  //       text2: isSetAlertError?.response,
  //       autoHide: true,
  //       visibilityTime: 4000,
  //     });
  //   }
  ...
  const handleSubmit = (values: any) => {
    ...
    AsyncStorage.getItem('userId').then(userId => {
      const reqParams = {
        ...
        timeframe: trade_timeframe === 'intraday' ? '15-min' : '1-d',
...
const AlertPopupIntraday = React.memo(PriceAlert);
export default AlertPopupIntraday;
